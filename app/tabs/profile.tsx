...
export default function Profile() {
  ...
  const loadPrivacyPolicy = async () => {
    if (privacyPolicyLoading || privacyPolicyText) {
      return;
    }
    try {
      setPrivacyPolicyLoading(true);
      const asset = Asset.fromModule(require('./Privacypolicy.html'));
      ...
      const sanitized = html
        ...
        .replace(/\n{2,}/g, '\n\n')
        .trim();
      setPrivacyPolicyText(sanitized);
    } catch (error) {
      console.error('[PROFILE] Failed to load privacy policy:', error);
      setPrivacyPolicyText(null);
    } finally {
  ...
  // Load privacy policy the first time the modal is opened
  useEffect(() => {
    if (showPrivacyPolicyModal) {
      loadPrivacyPolicy();
    }
  }, [showPrivacyPolicyModal]);
  const openExternalPrivacyPolicy = async () => {
    try {
      await WebBrowser.openBrowserAsync('https://voyageai.app/privacy');
    } catch (error) {
      console.error('[PROFILE] Unable to open privacy policy in browser', error);
      Alert.alert(t('Error'), t('Unable to open browser. Please try again later.'));
    }
  };
        ...
        <View style={styles.section}>
          ...
          {expandedSection === 'user' && (
            <View style={styles.sectionContent}>
              ...
              <TouchableOpacity style={styles.saveChangesButton} onPress={handleSaveUserInfo}>
            ...
            <View style={{ flex: 1 }}>
              {privacyPolicyLoading ? (
                <View style={styles.loadingContainer}>
                  ...
                  <Text style={styles.loadingLabel}>{t('Loading privacy policy...')}</Text>
                </View>
              ) : (
                <ScrollView style={styles.privacyScroll}>
                  <Text style={styles.privacyText}>
                    {privacyPolicyText ||
                      t('Unable to load the privacy policy locally. Use the button below to view it in your browser.')}
                  </Text>
                </ScrollView>
              )}
              <TouchableOpacity
                style={styles.privacyButton}
                onPress={openExternalPrivacyPolicy}
              >
                <Text style={styles.privacyButtonText}>
                  {t('Open full policy in browser')}
                </Text>
              </TouchableOpacity>
            </View>
          </View>
