...
function shouldPrioritizeInternational(query: string): boolean {
  ...
  try {
    ...
    if (query) {
      ...
      if (prioritizeInternational && mapboxPlace?.properties?.country_code === 'us') {
        ...
        const nonUSResults = mapboxResponse.data.features.filter((f: any) => 
          f.properties?.country_code !== 'us' && f.properties?.country_code !== 'US'
        );
    ...
    let photoAttributionUrl: string | undefined;
